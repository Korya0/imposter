...
class AppTransitions {
  ...
  /// If RTL (Arabic), it slides from the logical right (physical left).
  static CustomTransitionPage<void> slideFromRight({
    ...
    // 🌍 RTL Awareness: Flip the starting offset based on the locale
    ...
    return CustomTransitionPage<void>(
      ...
      transitionsBuilder: (context, animation, secondaryAnimation, child) {
        // RTL Logic: If RTL, start from -1 (left), otherwise 1 (right)
  ...
  /// If RTL (Arabic), it slides from the logical left (physical right).
  static CustomTransitionPage<void> slideFromLeft({
    ...
    return CustomTransitionPage<void>(
      ...
      reverseTransitionDuration: reverseDuration ?? _reverseDuration,
      transitionsBuilder: (context, animation, secondaryAnimation, child) {
        // RTL Logic: If RTL, start from 1 (right), otherwise -1 (left)
        final begin = isRtl ? const Offset(1, 0) : const Offset(-1, 0);
        const end = Offset.zero;
        const curve = Curves.easeInOutCubic;
  ...
  /// If RTL, the slide animation direction is logically flipped.
  static CustomTransitionPage<void> slideAndFade({
    ...
    // Default: Slide from logical right
    final defaultBegin = isRtl ? const Offset(-1, 0) : const Offset(1, 0);
    final effectiveBegin = begin ?? defaultBegin;
